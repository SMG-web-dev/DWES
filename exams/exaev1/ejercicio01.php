...
if (empty($_POST['token']) || empty($_SESSION['token']) || !hash_equals($_SESSION['token'], $_POST['token']))
    die('Solicitud inválida: token CSRF no válido.');

// Regenerar el token CSRF solo después de una validación exitosa
...
header("Location: acceso.php?msg=" . urlencode($_SESSION['msg']));
exit();
